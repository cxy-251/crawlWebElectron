...
from tests.fakes import UnusedSafari
...
class WorkflowTests(unittest.IsolatedAsyncioTestCase):
    ...
    def test_twitter_config_preserves_local_model_and_profile_url_input(self) -> None:
        ...
        config = yaml.safe_load((root / "configs" / "twitter.example.yaml").read_text(encoding="utf-8"))
        input_data = json.loads((root / "configs" / "twitter-target.example.json").read_text(encoding="utf-8"))
    ...
    def test_twitter_two_phase_configs_preserve_target_model_and_detail_defaults(self) -> None:
        root = Path(__file__).resolve().parents[2]
        input_data = json.loads((root / "configs" / "twitter-target.example.json").read_text(encoding="utf-8"))
        collect_config = yaml.safe_load((root / "configs" / "twitter.collect.example.yaml").read_text(encoding="utf-8"))
        clean_config = yaml.safe_load((root / "configs" / "twitter.clean.example.yaml").read_text(encoding="utf-8"))
