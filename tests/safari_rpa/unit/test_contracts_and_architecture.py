...
class ContractAndArchitectureTests(unittest.TestCase):
    def test_plan_archive_points_to_existing_current_plan(self) -> None:
        index = (ROOT / "docs" / "safari-rpa" / "runtime" / "plans" / "INDEX.md").read_text(encoding="utf-8")
        self.assertIn("0001-foundation.md", index)
        self.assertIn("0009-twitter-two-phase-collection-cleaning.md", index)
        self.assertTrue((ROOT / "docs" / "safari-rpa" / "runtime" / "plans" / "0001-foundation.md").is_file())
        self.assertTrue((ROOT / "docs" / "safari-rpa" / "runtime" / "plans" / "0009-twitter-two-phase-collection-cleaning.md").is_file())
        self.assertTrue((ROOT / "docs" / "safari-rpa" / "runtime" / "architecture" / "INDEX.md").is_file())
        self.assertTrue((ROOT / "docs" / "safari-rpa" / "runtime" / "architecture" / "design-principles.md").is_file())
    ...
    def test_http_routes_match_versioned_contract(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            application = build_application(Path(directory) / "route-test")
    ...
    def test_registry_exposes_boss_and_twitter_only(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            application = build_application(Path(directory) / "registry-test")
