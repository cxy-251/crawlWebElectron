...
from macrpa.contracts.runtime import JsonObject, RunRecord, RunStatus, StepStatus
from macrpa.contracts.safari import SafariAutomationPort
from macrpa.locking import AsyncFileLock
...
class WorkflowRunner:
    ...
    def start_background(self, run_id: str) -> None:
        ...
        if current and not current.done():
            return
        task = asyncio.create_task(self.execute(run_id), name=f"macrpa-run-{run_id}")
