import type { KuaishouUploadAdapter } from "../service/KuaishouUploadAdapter";
import type {
  KuaishouFormState,
  KuaishouOptionField,
...
import type { JsonRecord, LocalApiRequest, LocalApiRouteResult } from "../../../api/LocalApiTypes";
import { routeHandled, routeNotHandled } from "../../../api/LocalApiTypes";
...
export class KuaishouLocalApiRoutes {
  ...
  async handle({ request, method, url, readJson }: LocalApiRequest): Promise<LocalApiRouteResult> {
    ...
    if (method === "POST" && url.pathname === "/api/kuaishou/diagnostics/evidence") {
      return routeHandled(200, { ok: true, data: await this.adapter.captureDiagnosticEvidence() });
    }
